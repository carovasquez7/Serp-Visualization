...
#----PARA TRABAJAR CON D3----
#template vista htlm
@app.route('/api/v1.0/home')
def index():
    return render_template('index.html')

#/api/v1.0/getMyJson?filename=internalJsonGoogle.json
@app.route('/api/v1.0/getMyJson', methods=['GET'])
def getMyJson():
...
#----PARA TRABAJAR CON API Y MANIPULAR JSON----
...
#Para ejecutar se debe entregar el nombre del Nuevo atributo (numberWord) 
#"wordCountDescrip?newname=numberword&query=snippet"
...
def wordCountDescription():
    ...
    return jsonify({
        ...
        countWordDescription(lista[0], newnameattribute, queryname)
    })


@app.route('/api/v1.0/jsond3', methods=['GET'])
def jsonD3():   
    lista_final.append(formatJsonToD3(lista[0]))
    return jsonify({'dic_final': lista_final[0]})

    #return jsonify( formatJsonToD3(lista[0]))
...
def exportJson():
    ...
    namefile = "internalJsonGoogle_final"
    DictionaryToJson(lista_final[0], namefile)
    return jsonify({'Sucess': True})
